...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Function to add unique mouse IDs based on their most up{-}to{-}date cages}
\CommentTok{\# By using the most up{-}to{-}date cages, we can use an ID that will be easy }
\CommentTok{\# to use to identify a specific mouse at the current time (for example, }
\CommentTok{\# if you need to ID a mouse with a large change in weight that has happened}
\CommentTok{\# recently.)}
\CommentTok{\#}
\CommentTok{\# !! NEED TO CHANGE: Right now, it would miss mice that were sacrificed / died}
\CommentTok{\# !! before the latest time point that was collected}
\NormalTok{add\_latest\_mouse\_id }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(mouse\_weight\_data) \{}
  
\NormalTok{  mouse\_weight\_data }\OtherTok{\textless{}{-}}\NormalTok{ mouse\_weight\_data }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{current\_cage =} \FunctionTok{case\_when}\NormalTok{(}
\NormalTok{    date\_collected }\SpecialCharTok{!=} \FunctionTok{last}\NormalTok{(date\_collected) }\SpecialCharTok{\textasciitilde{}} \FunctionTok{as.character}\NormalTok{(}\ConstantTok{NA}\NormalTok{),}
    \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(new\_cage\_number) }\SpecialCharTok{\textasciitilde{}}\NormalTok{ new\_cage\_number, }
    \ConstantTok{TRUE} \SpecialCharTok{\textasciitilde{}}\NormalTok{ existing\_cage\_number}
\NormalTok{  ),}
         \AttributeTok{mouse\_id =} \FunctionTok{if\_else}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(current\_cage), }
                            \FunctionTok{paste}\NormalTok{(current\_cage, notch\_id, }\AttributeTok{sep =} \StringTok{"{-}"}\NormalTok{), }
                            \FunctionTok{as.character}\NormalTok{(}\ConstantTok{NA}\NormalTok{)))}
  \FunctionTok{return}\NormalTok{(mouse\_weight\_data)}
\NormalTok{\}}

\NormalTok{ex }\OtherTok{\textless{}{-}} \FunctionTok{add\_latest\_mouse\_id}\NormalTok{(ex)}

\NormalTok{ex }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{mouse\_id =} \FunctionTok{case\_when}\NormalTok{(}
    \CommentTok{\# Only create for the first time point, otherwise set to NA}
\NormalTok{    date\_collected }\SpecialCharTok{!=} \FunctionTok{first}\NormalTok{(date\_collected) }\SpecialCharTok{\textasciitilde{}} \ConstantTok{NA\_character\_}\NormalTok{,}
    \CommentTok{\# If the new cage number is missing, it means the animal did not }
    \CommentTok{\# change cages, so you can use the existing cage number}
    \FunctionTok{is.na}\NormalTok{(new\_cage\_number) }\SpecialCharTok{\textasciitilde{}} \FunctionTok{paste}\NormalTok{(existing\_cage\_number, notch\_id, }\AttributeTok{sep =} \StringTok{"{-}"}\NormalTok{),}
    \ConstantTok{TRUE} \SpecialCharTok{\textasciitilde{}} \FunctionTok{paste}\NormalTok{(new\_cage\_number, notch\_id, }\AttributeTok{sep =} \StringTok{"{-}"}\NormalTok{)}
\NormalTok{  ))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{add\_first\_mouse\_id }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(mouse\_weight\_data) \{}
...
\FunctionTok{add\_latest\_mouse\_id}\NormalTok{(ex)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Function to get the cage history of a mouse based on it\textquotesingle{}s latest mouse ID}
\CommentTok{\# and the mouse weight data. This works back through cage changes to track the }
\CommentTok{\# cages that a mouse has been in over the course of the experiment. }
\NormalTok{get\_cage\_history }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(latest\_mouse\_id, }
\NormalTok{                             mouse\_weight\_data) \{}
  
\NormalTok{  cage\_history }\OtherTok{\textless{}{-}}\NormalTok{ latest\_mouse\_id }\SpecialCharTok{\%\textgreater{}\%} 
    \FunctionTok{str\_remove}\NormalTok{(}\StringTok{"}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{{-}.+"}\NormalTok{)}
\NormalTok{  last\_cage }\OtherTok{\textless{}{-}}\NormalTok{ cage\_history[}\DecValTok{1}\NormalTok{]}
  
  \ControlFlowTok{while}\NormalTok{(}\FunctionTok{length}\NormalTok{(last\_cage) }\SpecialCharTok{\textgreater{}} \DecValTok{0}\NormalTok{)\{}
\NormalTok{    last\_cage }\OtherTok{\textless{}{-}}\NormalTok{ mouse\_weight\_data }\SpecialCharTok{\%\textgreater{}\%} 
      \FunctionTok{filter}\NormalTok{(new\_cage\_number }\SpecialCharTok{==}\NormalTok{ last\_cage) }\SpecialCharTok{\%\textgreater{}\%} 
      \FunctionTok{pull}\NormalTok{(existing\_cage\_number)}
    \ControlFlowTok{if}\NormalTok{(}\FunctionTok{length}\NormalTok{(last\_cage) }\SpecialCharTok{\textgreater{}} \DecValTok{0}\NormalTok{) \{}
\NormalTok{      cage\_history }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(cage\_history, last\_cage)}
\NormalTok{    \}}
\NormalTok{  \}}
  
\NormalTok{  cage\_history }\OtherTok{\textless{}{-}} \FunctionTok{unique}\NormalTok{(cage\_history)}
  
  \FunctionTok{return}\NormalTok{(cage\_history)}
\NormalTok{\}}

\FunctionTok{get\_cage\_history}\NormalTok{(}\AttributeTok{latest\_mouse\_id =} \StringTok{"22009A{-}0"}\NormalTok{, }
                 \AttributeTok{mouse\_weight\_data =}\NormalTok{ ex)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ex\_cage\_history }\OtherTok{\textless{}{-}} \FunctionTok{get\_cage\_history}\NormalTok{(}\AttributeTok{latest\_mouse\_id =} \StringTok{"22009A{-}0"}\NormalTok{, }
                 \AttributeTok{mouse\_weight\_data =}\NormalTok{ ex)}


\CommentTok{\# Use the function to get cage histories for all the mice}
\NormalTok{latest\_mouse\_ids }\OtherTok{\textless{}{-}}\NormalTok{ ex }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{filter}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(mouse\_id)) }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{pull}\NormalTok{(mouse\_id) }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{unique}\NormalTok{()}
  
\NormalTok{mouse\_cage\_histories }\OtherTok{\textless{}{-}}\NormalTok{ purrr}\SpecialCharTok{::}\FunctionTok{map}\NormalTok{(latest\_mouse\_ids, get\_cage\_history, }
                                   \AttributeTok{mouse\_weight\_data =}\NormalTok{ ex)}
\FunctionTok{names}\NormalTok{(mouse\_cage\_histories) }\OtherTok{\textless{}{-}}\NormalTok{ latest\_mouse\_ids}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{add\_one\_mouse\_id }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(latest\_mouse\_id, cage\_history, mouse\_weight\_data) \{}
  
\NormalTok{  mouse\_notch }\OtherTok{\textless{}{-}} \FunctionTok{str\_remove}\NormalTok{(latest\_mouse\_id, }\StringTok{".+}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{{-}"}\NormalTok{)}
  
\NormalTok{  mouse\_weight\_data }\OtherTok{\textless{}{-}}\NormalTok{ mouse\_weight\_data }\SpecialCharTok{\%\textgreater{}\%} 
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{mouse\_id =} \FunctionTok{if\_else}\NormalTok{((existing\_cage\_number }\SpecialCharTok{\%in\%}\NormalTok{ cage\_history) }\SpecialCharTok{\&} 
\NormalTok{                                notch\_id }\SpecialCharTok{==}\NormalTok{ mouse\_notch, }
\NormalTok{           latest\_mouse\_id, }
\NormalTok{           mouse\_id))}
  
  \FunctionTok{return}\NormalTok{(mouse\_weight\_data)}
\NormalTok{\}}

\NormalTok{ex2 }\OtherTok{\textless{}{-}} \FunctionTok{add\_one\_mouse\_id}\NormalTok{(}\AttributeTok{latest\_mouse\_id =} \FunctionTok{names}\NormalTok{(mouse\_cage\_histories)[}\DecValTok{1}\NormalTok{], }
                 \AttributeTok{cage\_history =}\NormalTok{ mouse\_cage\_histories[[}\DecValTok{1}\NormalTok{]], }
                 \AttributeTok{mouse\_weight\_data =}\NormalTok{ ex)}

\NormalTok{ex2 }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{filter}\NormalTok{(existing\_cage\_number }\SpecialCharTok{==} \StringTok{"22003"} \SpecialCharTok{\&}\NormalTok{ notch\_id }\SpecialCharTok{==} \StringTok{"0"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\FunctionTok{length}\NormalTok{(mouse\_cage\_histories)) \{}
\NormalTok{  ex }\OtherTok{\textless{}{-}} \FunctionTok{add\_one\_mouse\_id}\NormalTok{(}\AttributeTok{latest\_mouse\_id =} \FunctionTok{names}\NormalTok{(mouse\_cage\_histories)[i], }
                         \AttributeTok{cage\_history =}\NormalTok{ mouse\_cage\_histories[[i]], }
                         \AttributeTok{mouse\_weight\_data =}\NormalTok{ ex) }
\NormalTok{\}}
...
\NormalTok{ex }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ date\_collected, }\AttributeTok{y =}\NormalTok{ weight, }\AttributeTok{group =}\NormalTok{ mouse\_id, }\AttributeTok{color =}\NormalTok{ sex)) }\SpecialCharTok{+} 
  \FunctionTok{geom\_line}\NormalTok{() }\SpecialCharTok{+} 
  \FunctionTok{facet\_wrap}\NormalTok{(}\SpecialCharTok{\textasciitilde{}}\NormalTok{ group)}
\end{Highlighting}
\end{Shaded}

\includegraphics{csu-impactb_files/figure-latex/unnamed-chunk-8-1.pdf}
...
\includegraphics{csu-impactb_files/figure-latex/unnamed-chunk-8-2.pdf}
...
\includegraphics{csu-impactb_files/figure-latex/unnamed-chunk-8-3.pdf}
...
\NormalTok{dataset }\OtherTok{\textless{}{-}}\NormalTok{ data}\SpecialCharTok{$}\NormalTok{before\_vaccination }\SpecialCharTok{\%\textgreater{}\%}
 \FunctionTok{select}\NormalTok{(}\StringTok{"sex"}\NormalTok{, }\StringTok{"mouse\_number"}\NormalTok{, }\StringTok{"weight"}\NormalTok{, }\StringTok{"existing\_cage\_number"}\NormalTok{, }\StringTok{"group"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{csu-impactb_files/figure-latex/unnamed-chunk-21-1.pdf}
...
\hypertarget{create-a-fitted-sigmoid-model-function-and-apply-the-function-to-the-data}{%
\subsubsection{Create a fitted sigmoid model function and apply the function to the data}\label{create-a-fitted-sigmoid-model-function-and-apply-the-function-to-the-data}}
...
\includegraphics{csu-impactb_files/figure-latex/unnamed-chunk-27-1.pdf}
...
\begin{center}\includegraphics{csu-impactb_files/figure-latex/unnamed-chunk-33-1} \end{center}
...
\includegraphics{csu-impactb_files/figure-latex/unnamed-chunk-41-1.pdf}
...
\includegraphics{csu-impactb_files/figure-latex/unnamed-chunk-54-1.pdf}
