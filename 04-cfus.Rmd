...
tidy_cfu_data <- all_data %>%
  mutate(dilution = str_extract(dilution, "[0-9]+"),
         dilution = as.numeric(dilution)) %>%
  filter((CFUs >= 5 & CFUs <= 95) | group == "Control") %>%
...
tidy_cfu_meta_joined <- inner_join(meta, tidy_cfu_data) %>%
  ...
  select(organ, count_date, who_plated, who_counted, group,  mouse, dilution,  
         CFUs, CFUs_per_ml) %>% 
  ungroup()
...
tidy_lung_cfu_plot <- tidy_cfu_meta_joined %>%
  filter(organ == "lung") %>%
  mutate(group = fct_relevel(group, "Inoculum",
            "Control", "Day_3", "Day_7",
            "Day_14", "Day_28", "Day_39", "Day_63")) %>%
  ggplot(aes(x = group, y = log10(CFUs_per_ml), fill = group))+
  stat_boxplot( aes(x = group, y = log10(CFUs_per_ml)), 
    geom='errorbar', linetype=1, width=0.5)+ 
  geom_boxplot(aes(group = group), fill = NA, show.legend = FALSE, color = "lightgrey")+
