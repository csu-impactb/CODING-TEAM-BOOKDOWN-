...
ELISA data can be analyzed in different ways based on how the data is acquired [@hartman2018absorbance]. There are a a few examples of the type of ELISA data :
...
## Create a merge function of Fitted model and endpoint titer
...
elisa_data_with_fit_model <- fitted_model_elisa(elisa_data_nested$data[[2]], start_a = 4, start_d = 0, start_c = -8, start_b = 1) 

# Fitting the model into the whole dataframe

elisa_data_with_fit_model_whole <- elisa_data_nested %>%
  mutate(fitted_data = purrr::map(data, ~ fitted_model_elisa(.x, start_a = 4, start_d = 0, start_c = -8, start_b = 1)))

elisa_data_with_fit_model_whole
...
endpoint_titer_elisa(elisa_data_with_fit_model, back_value = 0.2)

# Taking output of the fitted model function and input it into endpoint titer function

elisa_data_with_endpoint_titer <- elisa_data_with_fit_model_whole %>%
